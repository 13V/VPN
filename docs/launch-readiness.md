...
| --- | --- | --- |
| Website and sample portal | **Ready as a prototype.** Deployed at https://vpn-one-phi.vercel.app; the current offline suite includes the pilot tests. | Visitors can explore sample plans and simulated connection states. No VPN or payment is activated by the demo. |
| Private prepaid VPN pilot | **Code ready; provider setup and connection test outstanding.** A wallet-bound grant can retrieve a real reseller WireGuard config once an operator has funded and provisioned one capped account. | Do not advertise general availability. A private tester may use the real setup only after its account, expiry and connection are verified. |
...
The preferred sequence is a transparent prototype, a separately funded one-account VPN pilot, then a separately reviewed token release and wider service after the supplier and funding gates pass. If the token releases before the general VPN benefit, the launch page and social copy must say that the benefit is **in development**, with no date or guaranteed amount.
...
- A separate [private live pilot](live-pilot.md) uses VPNresellers prepaid credit and a repeatable WireGuard configuration GET instead of Blink per-order payment. Published reseller pricing and crypto top-ups have been reviewed, but no reseller account, paid order or live tunnel result is available yet.
